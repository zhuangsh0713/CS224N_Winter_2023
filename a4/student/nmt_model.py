...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.post_embed_cnn = torch.nn.Conv1d(embed_size, embed_size, 2, padding='same')
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        ###
        last_hidden_reshaped = torch.cat((last_hidden[0], last_hidden[1]), dim=1)  # shape: (b, 2*h)
        init_decoder_hidden = self.h_projection(last_hidden_reshaped)  # shape: (b, h)

        last_cell_reshaped = torch.cat((last_cell[0], last_cell[1]), dim=1)  # shape: (b, 2*h)
        init_decoder_cell = self.c_projection(last_cell_reshaped)  # shape: (b, h)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1, dim=0):
            ...
            state, o_t, _ = self.step(Ybar_t, enc_hiddens=enc_hiddens, enc_hiddens_proj=enc_hiddens_proj, dec_state=dec_state, enc_masks=enc_masks)
